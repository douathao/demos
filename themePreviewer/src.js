require([
	'dojo/io-query', 'dojo/query', 'dojo/on', 'dojo/_base/array', 'dojo/number', 'dojo/_base/fx',
	'dojo/dom', 'dojo/dom-class', 'dojo/dom-geometry', 'dojo/dom-style',
	...
	// programmatically dijit
	'dijit/form/FilteringSelect',
	'dijit/InlineEditBox',
	'dijit/Tree',

	// Editors used by InlineEditBox.  Must be pre-loaded.
	'dijit/form/Textarea', 'dijit/form/DateTextBox', 'dijit/form/TimeTextBox',
	...
	'dijit/layout/TabContainer', 'dijit/form/ComboButton', 'dijit/form/ToggleButton',
	...
	'dijit/form/HorizontalRule', 'dijit/TitlePane', 'dijit/ProgressBar', 'dojo/dnd/Source',
	'dijit/Dialog',

	// Don't call the parser until the DOM has finished loading
	'dojo/domReady!'
], function(
	ioQuery, query, on, array, number, baseFx, dom, domClass, domGeom, domStyle, has, locale, parser, Memory,
	registry, ObjectStoreModel, CheckedMenuItem, RadioMenuItem, MenuSeparator, DstoreMemory, OnDemandGrid,
	Chart, Pie, Highlight, MoveSlice, Tooltip, MiamiNice, Legend, FilteringSelect, InlineEditBox, Tree) {
	...
	var data = [
			...
			{ id: 'MX', name: 'Mexico', type: 'country', population: '108 million', area: '1,972,550 sq km',
				parent: 'NA' },
			{ id: 'Mexico City', name: 'Mexico City', type: 'city',
				population: '19 million', timezone: '-6 UTC', parent: 'MX'},
			{ id: 'Guadalajara', name: 'Guadalajara', type: 'city',
				population: '4 million', timezone: '-6 UTC', parent: 'MX' },
			{ id: 'CA', name: 'Canada', type: 'country',
				population: '33 million', area: '9,984,670 sq km', parent: 'NA' },
			{ id: 'Ottawa', name: 'Ottawa', type: 'city',
				population: '0.9 million', timezone: '-5 UTC', parent: 'CA'},
			{ id: 'Toronto', name: 'Toronto', type: 'city',
				population: '2.5 million', timezone: '-5 UTC', parent: 'CA' },
	...
	if (queries.theme) {
		...
		theme = queries.theme;
		if (queries.dir) {
			query('link[data-stylesheet=rtl]')[0].href = '../../dijit/themes/' + theme + '/' + theme + '_rtl.css';
		}
	}
	else {
		// default to claro
	...
	query('link[data-stylesheet=document]')[0].href = '../../dijit/themes/' + theme + '/document.css';
	query('link[data-stylesheet=dijit]')[0].href = '../../dijit/themes/' + theme + '/' + theme + '.css';
	// dgrid
	query('link[data-stylesheet=dgrid]')[0].href = '../../dgrid/css/skins/' + theme + '.css';
	...
	parser.parse(document.body).then(function(){
		...
		new InlineEditBox({
			...
			editorParams: {
				store: continentStore,
				autoComplete: true,
				promptMessage: 'Please enter a place'
			},
			width:'300px'
		}, 'backgroundArea2');
		...
		// Events
		on(dom.byId('contextMenuEnable'), 'click', function () {
			alert('Hello world');
		});
		query('.consoleLog').on('click', function (event) {
			console.log(event.target.parentNode.getAttribute('data-message'));
		});
		query('.subMenu', registry.byId('popupContextMenu').domNode).on('click', function (event) {
			alert(event.target.innerHTML + '!');
		});
		query('.showLoading').on('click', showDialog);
		query('.actionBar').on('click', showDialogAb);
		on(dom.byId('inputPaddingDefault'), 'click', setTextBoxPadding);
		on(dom.byId('inputPadding1'), 'click', setTextBoxPadding);
		on(dom.byId('inputPadding2'), 'click', setTextBoxPadding);
		on(dom.byId('inputPadding3'), 'click', setTextBoxPadding);
		on(dom.byId('inputPadding4'), 'click', setTextBoxPadding);
		on(dom.byId('inputPadding5'), 'click', setTextBoxPadding);
		on(registry.byId('colorPalette'), 'change', setBackground);
		on(registry.byId('colorPalette2'), 'change', setBackground);
		on(dom.byId('simpleButton'), 'click', function () {
			console.debug('clicked simple');
		});
		on(dom.byId('editMenu1'), 'click', function () {
			console.debug('not actually cutting anything, just a test!');
		});
		on(dom.byId('editMenu2'), 'click', function () {
			console.debug('not actually copying anyything, just a test!');
		});
		on(dom.byId('editMenu3'), 'click', function () {
			console.debug('not actually pasting anyything, just a test!');
		});
		on(dom.byId('editMenu4'), 'click', function () {
			console.log('clicked combo save');
		});
		on(dom.byId('saveMenu1'), 'click', function () {
			console.debug('not actually saving anything, just a test!');
		});
		on(dom.byId('saveMenu2'), 'click', function () {
			console.debug('not actually saving anything, just a test!');
		});
		on(registry.byId('toggleButton'), 'change', function (a) {
			console.log('toggle button checked=' + a);
		});
		on(dom.byId('ABdialog1button2'), 'click', function () {
			dialogAB.onCancel();
		});
		on(registry.byId('slider2'), 'change', function (a) {
			dom.byId('slider2input').value = a;
		});
		on(registry.byId('horizontal1'), 'change', function (a) {
			dom.byId('slider1input').value = number.format(a / 100, { places:1, pattern:'#%' });
		});
		on(dom.byId('disableEditable'), 'click', function () {
			areaEditable.set('disabled', true);
		});
		on(dom.byId('enableEditable'), 'click', function () {
			areaEditable.set('disabled', false);
		});

		registry.byId('topTabs').watch('selectedChildWidget', function(name, oval, nval){
		...
		// It's the server's responsibility to localize the date displayed
		// in the (non-edit) version of an InlineEditBox,
